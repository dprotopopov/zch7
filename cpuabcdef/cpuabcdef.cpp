...
static const size_t _n = 6;
static const size_t _m[] = {5, 5, 5, 5, 5, 5};
static const double _a[] = {0, 0, 0, 0, 0, 0};
static const double _b[] = { 1000, 1000, 1000, 1000, 1000, 1000 };
...
#pragma omp parallel for
	for (size_t i = 0; i < s.size(); i++)
	{
		...
		double y1 = h.a*x[0] + h.a*h.b*(x[1] + h.c*x[2] + h.d*x[3] + h.e*x[4] + h.e*h.f*x[5]);
		s[i] = (h.y - y1)*(h.y - y1);
	}
...
int main(int argc, char* argv[])
{
	...
	while (std::getline(std::cin, line))
	{
		...
		std::stringstream lineStream(line);
		history_t h;
		lineStream >> h.a >> h.b >> h.c >> h.d >> h.e >> h.f >> h.y;
		history.push_back(h);
	}
